          ...
          <v-dialog v-model="showDelDialog" max-width="500px">
            <v-card>
              <v-card-title class="text-h5"
                >Are you sure you want to delete this Category with associated
                data?</v-card-title
              >
              <v-card-actions>
                <v-spacer></v-spacer>
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              class="ma-1"
              @click.stop="editCategoryFunc(item)"
              x-small
              color="neutral"
              fab
              dark
...
export default {
  name: "CategoriesList",
  components: { CategoriesEditDialog },
  data: () => ({
    search: "",
  ...
  methods: {
    ...
    deleteCategories(id) {
      ...
      let uri = `http://` + window.location.hostname + `:4000/category/${id}`;
      axios
        .delete(uri, data, this.categoriesList)
        .then(() => {
          this.refreshList();
        })
        .catch((error) => {
    ...
    editCategoryFunc(categoryTemp) {
      this.isEdit = true;
      this.editCategory = categoryTemp;
    ...
    deleteItemConfirm() {
      this.deleteCategories(this.deleteid);
      this.closeDelete();
    },
